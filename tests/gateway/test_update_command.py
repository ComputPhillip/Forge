...
class TestHandleUpdateCommand:
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when hermes is not on PATH and forge_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m forge_cli.main when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        # The update_cmd uses sys.executable -m forge_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "forge_cli.main" in joined or "bash" in call_args[0]
    ...
    async def test_resolve_hermes_bin_fallback(self):
        ...
        assert result == [sys.executable, "-m", "forge_cli.main"]
