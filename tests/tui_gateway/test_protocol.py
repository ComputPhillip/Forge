...
def server():
    with patch.dict("sys.modules", {
        "forge_constants": MagicMock(get_hermes_home=MagicMock(return_value="/tmp/hermes_test")),
        "forge_cli.env_loader": MagicMock(),
        "forge_cli.banner": MagicMock(),
        "forge_state": MagicMock(),
    }):
        import importlib
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "forge_cli.plugins.get_plugin_command_handler",
