...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import forge_constants; print(forge_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("forge_constants.py", result["output"])
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_returns_code_execution_section(self):
        from tools.code_execution_tool import _load_config
        with patch("forge_cli.config.read_raw_config",
    ...
    def test_does_not_import_interactive_cli(self):
        ...
        with patch.dict("sys.modules", {"cli": mock_cli}), \
             patch("forge_cli.config.read_raw_config", return_value={}):
            result = _load_config()
