...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "forge_cli"}
        or name.startswith("tools.")
        or name.startswith("agent.")
        or name.startswith("forge_cli.")
    }
    try:
        yield
    finally:
        _reset_modules(("tools", "agent", "forge_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    forge_cli = types.ModuleType("forge_cli")
    forge_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["forge_cli"] = forge_cli
    sys.modules["forge_cli.config"] = types.SimpleNamespace(
