"""Tests for forge_cli.gateway_windows."""

from pathlib import Path

import pytest

import forge_cli.gateway as gateway
import forge_cli.gateway_windows as gateway_windows
import forge_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("forge_cli.config.get_hermes_home", lambda: str(tmp_path / "hermes-home"))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "forge_cli.main"]
