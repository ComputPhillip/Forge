...
# forge_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``forge_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see forge_cli/kanban_db.py) would
...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``forge_state._set_last_init_error(None)`` explicitly.
