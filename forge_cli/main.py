...
# IMPORTANT: forge_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``forge_bootstrap`` is a
...
# new code references ``forge_bootstrap`` but the editable install's
...
try:
    import forge_bootstrap  # noqa: F401
...
def _print_fast_version_info() -> None:
    from forge_cli import __release_date__, __version__
...
def _apply_profile_override() -> None:
    ...
    # Mirrors forge_cli.profiles._PROFILE_ID_RE so we never call
    ...
    if profile_name is None:
        try:
            from forge_constants import get_default_hermes_root
    ...
    if profile_name is not None:
        try:
            from forge_cli.profiles import resolve_profile_env
...
from forge_cli.config import get_hermes_home
from forge_cli.env_loader import load_hermes_dotenv

load_hermes_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERMES_REDACT_SECRETS env
# var BEFORE forge_logging imports agent.redact (which snapshots the flag at
...
try:
    from forge_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from forge_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if forge_constants not importable yet
...
from forge_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from forge_cli.config import get_env_path, get_hermes_home, load_config
    from forge_cli.auth import get_auth_status

    # Determine whether Hermes itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from forge_cli.config import DEFAULT_CONFIG
    ...
    from forge_cli.auth import PROVIDER_REGISTRY
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from forge_state import SessionDB
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from forge_cli.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from forge_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    if code == 42:
        from forge_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from forge_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    try:
        ...
        from forge_cli.config import load_config as _load_config_for_xai_check
    ...
    if _termux_should_prefetch_update_check():
        try:
            from forge_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from forge_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from forge_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from forge_cli.config import get_env_value, save_env_value
...
def cmd_setup(args):
    ...
    from forge_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from forge_cli.config import stamp_install_method
    from forge_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    from forge_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from forge_cli.providers import resolve_provider_full
    ...
    from forge_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _clear_stale_openai_base_url():
    ...
    """
    from forge_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`forge_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from forge_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from forge_cli.config import load_config, save_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from forge_cli.config import load_config
    from forge_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from forge_cli.auth import _prompt_model_selection
    from forge_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from forge_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from forge_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from forge_constants import OPENROUTER_BASE_URL
    from forge_cli.auth import (
        ProviderConfig,
    ...
    from forge_cli.config import get_env_value
    ...
    from forge_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from forge_constants import AI_GATEWAY_BASE_URL
    from forge_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from forge_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from forge_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from forge_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from forge_cli.codex_models import get_codex_model_ids
    ...
    if not _codex_token:
        try:
            from forge_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from forge_cli.models import _PROVIDER_MODELS
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from forge_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from forge_cli.auth import _save_model_choice, deactivate_provider
    from forge_cli.config import get_env_value, load_config, save_config
    from forge_cli.secret_prompt import masked_secret_prompt
    ...
    from forge_cli.models import probe_api_models
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from forge_cli.runtime_provider import _detect_api_mode_for_url
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from forge_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from forge_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from forge_cli import azure_detect
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from forge_cli.curses_ui import flush_stdin
...
# downstream call sites read `forge_cli.main._PROVIDER_MODELS` directly,
...
def _model_flow_copilot(config, current_model=""):
    ...
    from forge_cli.config import save_env_value, load_config, save_config
    from forge_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from forge_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from forge_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from forge_cli.env_loader import format_secret_source_suffix
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    """
    from forge_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from forge_cli.config import save_env_value
    from forge_cli.secret_prompt import masked_secret_prompt
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from forge_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        from forge_cli.auth import AuthError
        from forge_cli.models import fetch_lmstudio_models
        ...
        from forge_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from forge_constants import display_hermes_home as _dhh_fn
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from forge_cli.auth import get_anthropic_key
...
def cmd_login(args):
    ...
    from forge_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from forge_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from forge_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from forge_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from forge_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from forge_cli.webhook import webhook_command
...
def cmd_portal(args):
    ...
    from forge_cli.portal_cli import portal_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from forge_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from forge_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from forge_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from forge_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from forge_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from forge_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from forge_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from forge_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from forge_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from forge_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from forge_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from forge_cli.banner import check_for_updates
        from forge_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from forge_cli.uninstall import run_uninstall
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``forge_cli/web_dist/`` (per vite.config.ts
    outDir: "../forge_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "forge_cli" / "web_dist"
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m forge_cli.main web` invocation.
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    patterns = [
        "hermes dashboard",
        "forge_cli.main dashboard",
        "forge_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `forge_cli.gateway._scan_gateway_pids` and avoids the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # forge_cli.config.get_hermes_home to simulate setup failure.
        from forge_cli.config import get_hermes_home as _get_hermes_home
...
def _cmd_update_check():
    ...
    from forge_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from forge_cli.config import recommended_update_command
        from forge_cli.banner import check_via_pypi
...
def _run_pre_update_backup(args) -> None:
    ...
    try:
        from forge_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    try:
        from forge_constants import get_hermes_home, display_hermes_home
...
def cmd_update(args):
    ...
    """
    from forge_cli.config import is_managed, managed_error
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        try:
            from forge_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in forge_cli/config.py bricked
        ...
        # the old bytecode (e.g. get_hermes_home added to forge_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload forge_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_hermes_home() added since the last release.
        try:
            import importlib
            import forge_constants as _hc
        ...
        try:
            from forge_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from forge_cli.tools_config import install_cua_driver
...
def cmd_profile(args):
    ...
    from forge_constants import display_hermes_home
    ...
    if action == "list":
        ...
        from forge_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from forge_constants import get_hermes_home as _hh
        ...
        from forge_cli import profile_describer as _pd
        ...
        from forge_cli.profiles import profile_exists
        ...
        from forge_cli.profiles import rename_profile
        ...
        from forge_cli.profiles import export_profile
        ...
        from forge_cli.profiles import import_profile
        ...
        import tempfile
        from forge_cli.profile_distribution import (
            plan_install,
        ...
        from forge_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from forge_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from forge_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def cmd_dashboard(args):
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "forge_cli" / "web_dist"
        )
    ...
    from forge_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from forge_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from forge_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from forge_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``forge_cli/_parser.py``.
#
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        from forge_cli.plugins import discover_plugins
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from forge_cli._parser import build_top_level_parser
...
def main():
    ...
    try:
        from forge_cli.stdio import configure_windows_stdio
    ...
    # =========================================================================
    from forge_cli.fallback_cmd import cmd_fallback
    ...
    from forge_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from forge_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    from forge_cli.send_cmd import register_send_subparser
    ...
    # =========================================================================
    from forge_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from forge_cli.kanban import build_parser as _build_kanban_parser
...
""",
    ...
    from forge_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    def cmd_pairing(args):
        from forge_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from forge_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from forge_cli.skills_hub import skills_command
    ...
    from forge_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    def cmd_plugins(args):
        from forge_cli.plugins_cmd import plugins_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from forge_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from forge_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from forge_cli.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from forge_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from forge_cli.tools_config import tools_command
    ...
    def cmd_mcp(args):
        from forge_cli.mcp_config import mcp_command
    ...
    def cmd_claw(args):
        from forge_cli.claw import claw_command
...
""",
    ...
    from forge_cli.config import get_container_exec_info
