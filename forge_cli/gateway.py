...
# when forge_constants is cached from a pre-update version during `hermes update`.
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "forge_cli.main gateway",
        "forge_cli.main --profile",
        "forge_cli.main -p",
        "forge_cli/main.py gateway",
        "forge_cli/main.py --profile",
        "forge_cli/main.py -p",
        "hermes gateway",
        "gateway/run.py",
    ]
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from forge_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "forge_cli.main"]
    if profile != "default":
...
def _gateway_list() -> None:
    ...
    """
    try:
        from forge_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
from forge_constants import is_container, is_termux, is_wsl
...
User={username}
Group={group_name}
ExecStart={python_path} -m forge_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def _setup_whatsapp():
    ...
    from forge_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from forge_cli.dingtalk_auth import dingtalk_qr_auth
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``forge_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from forge_cli import setup as _s
    return {
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`forge_cli.service_manager` are caught and surfaced as
