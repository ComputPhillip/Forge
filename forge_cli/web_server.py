...
Usage:
    python -m forge_cli.main web          # Start on http://127.0.0.1:9119
    python -m forge_cli.main web --port 8080
"""
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``forge_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "forge_cli.main", *subcommand]
...
# in forge_cli/config.py — listed here for deterministic ordering in the UI.
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from forge_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from forge_cli.auth import clear_provider_auth
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from forge_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from forge_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from forge_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from forge_cli import profiles as profiles_mod
    try:
...
async def get_skills():
    ...
    from forge_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from forge_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from forge_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    matching what ``forge_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from forge_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as forge_cli.plugins):
    ...
    from forge_cli.plugins import get_bundled_plugins_dir
    ...
    # ``forge_cli/plugins.py`` and the documented user contract.
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from forge_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from forge_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from forge_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from forge_cli.plugins_cmd import dashboard_remove_user_plugin
