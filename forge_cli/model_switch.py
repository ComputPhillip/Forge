...
- ``forge_cli.providers``        -- canonical provider identity + overlays
- ``forge_cli.model_normalize``  -- per-provider name formatting
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from forge_cli.model_switch import
...
def switch_model(
    ...
    # Mirrors the same logic in forge_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from forge_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from forge_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from forge_cli.auth import _load_auth_store
    ...
    from forge_cli.providers import HERMES_OVERLAYS
    from forge_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    # with `hermes model`).
    try:
        from forge_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # (see forge_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, forge_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`forge_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from forge_cli.models import fetch_openrouter_models
