...
from forge_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

# Forge logo — same block-letter style and gold/bronze gradient as the
# upstream HERMES AGENT logo, so users coming from hermes-agent see a
# visually consistent banner. The HERMES_AGENT_LOGO name is preserved
# as a module attribute purely so existing imports / skin lookups don't
# break during the surface-rebrand transition.
FORGE_LOGO = """[bold #FFD700]███████╗ ██████╗ ██████╗  ██████╗ ███████╗[/]
[bold #FFD700]██╔════╝██╔═══██╗██╔══██╗██╔════╝ ██╔════╝[/]
[#FFBF00]█████╗  ██║   ██║██████╔╝██║  ███╗█████╗  [/]
[#FFBF00]██╔══╝  ██║   ██║██╔══██╗██║   ██║██╔══╝  [/]
[#CD7F32]██║     ╚██████╔╝██║  ██║╚██████╔╝███████╗[/]
...
# Backward-compat alias for downstream code / skins that reference the
# upstream name. Both point to the same Forge logo.
HERMES_AGENT_LOGO = FORGE_LOGO
...
def format_banner_version_label() -> str:
    ...
    Displays Forge's own version, with the upstream hermes-agent version
    appended in parens so users always know which engine version they're
    on. Honest about the fork relationship.
    """
    base = f"Forge v0.1.0  (engine: hermes-agent v{VERSION}, {RELEASE_DATE})"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        from forge_cli.codex_runtime_switch import get_current_runtime
        from forge_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from forge_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
