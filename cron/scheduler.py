...
# the module) fail with ModuleNotFoundError for forge_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from forge_constants import get_hermes_home
from forge_cli._subprocess_compat import windows_hide_flags
from forge_cli.config import load_config, _expand_env_vars
from forge_time import now as _hermes_now
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    if per_job:
        return per_job
    try:
        from forge_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    ...
    from forge_cli.profiles import normalize_profile_name, resolve_profile_env
    from forge_constants import reset_hermes_home_override, set_hermes_home_override
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    * anything else — run with the current Python interpreter
    ...
    try:
        from forge_constants import get_subprocess_home
