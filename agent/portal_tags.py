...
The version is sourced live from ``forge_cli.__version__`` so it auto-aligns
...
``forge_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``forge_cli`` cannot be imported (should
