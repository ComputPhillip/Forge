...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`forge_cli.plugins.invoke_hook` and its aggregators.  Python
...
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``forge_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``forge_cli.config.load_config``
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from forge_cli.plugins import VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`forge_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`forge_cli.plugins.invoke_hook`
